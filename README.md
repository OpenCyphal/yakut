...
Then, if you are using GNU/Linux, make sure to install these:
```bash
...
```bash
YAKUT_PATH=.nox/test-3-8/tmp/.compiled/ python -m yakut --format=TSVH subscribe 142:reg.udral.physics.dynamics.rotation.PlanarTs.0.1 > rotation_data.tsv
```
